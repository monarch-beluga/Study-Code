...
def create_lapgrd(f, t):
    ...
    for i in range(count_grd):
        res_name.append(t + os.sep + outpath[0] + os.sep + f'{t}_{f[-8:-4]}{day:03d}')
        shutil.copy(dem.split('.')[0] + '.prj', res_name[i] + '.prj')
        cov_name.append(t + os.sep + outpath[1] + os.sep + f'{t}_{f[-8:-4]}{day:03d}_COV')

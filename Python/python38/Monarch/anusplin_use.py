...
# -----------------插值配置------------------------
types = ['DMNT', 'DMXT', 'PREP', 'AVRH', 'SOHR', 'MEWS']                # 插值的要素
out_pre = ['TMIN', 'TMAX', 'PRCP', 'RHU', 'SSD', 'WIN']
# out_pre = ['PRCP', 'TMIN', 'TMAX', 'WIN', 'RHU', 'SSD']
q = 0.8                                 # 数据完整性百分比
...
def select_data():
    ...
    df = pd.read_csv(r'CH_2018.txt', header=0)
    df = df
...
def create_splina(t, f, dem_min, dem_max, pre):
    ...
    with open(f'{pre}/splina00000.cmd', 'w') as fp:
...
def create_lapgrd(f, pre):
    ...
    for i in range(count_grd):
        ...
        day += sep_day
    with open(f'{pre}/lapgrd00000.cmd', 'w') as fp:

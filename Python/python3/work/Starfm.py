...
def makeindexs(x):
    index = (np.arange(x, x + window)[:, None] + long * np.arange(window)[None, :]).T.reshape(window * window)
    return index
...
def starfm(landsat0, modis0, modis1, w, times):
    ...
    left, one = makedivid(up * right, times, right)
...
def dividstarfm(one, left, l0, m0, m1, right, up, newright, newup, w, d):
    ...
    ##get windowindexs
    windowindex = loopindex(left, one, right)
...
landsatfir = 's2_2022_11_07.tif'
modisfir = 'modis_2022_11_07.tif'
aimlandsatfir = 'L72002311_SZ_B432_30m.tif'
aimmodisfir = 'modis_2022_08_09.tif'

with rasterio.open(landsatfir) as src:
    landsat0 = src.read(1)[::-1]
# modis0 = gdal.Open(modisfir).ReadAsArray()[0]
with rasterio.open(modisfir) as src:
    profile = src.profile
    modis0 = src.read(1)
# landsat0 = gdal.Open(landsatfir).ReadAsArray()[0]
with rasterio.open(aimmodisfir) as src:
    modis1 = src.read(1)
...
l1 = starfm(landsat0, modis0, modis1, window, 30)

file = r'L72000306_SZ_B432_30m.tif'
outfile = r'modis_to_s2_2022_11_07.tif'

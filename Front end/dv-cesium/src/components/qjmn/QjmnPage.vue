...
const tabs = [
  ...
  //   index:4,
  //   title: "情景模拟四",
  //   content: "重金属废水泄漏:安义工业园区化工集中区东阳片区二某企业因设备故障，如若输送管道破裂，铊废水流入雨水管网，如不采取措施，将造成污染事故，影响到潦河下游水质。"
  // },
]

function FunClick(item){
  panelDisplay.value = false
  let path = staticData.api + staticData.qjmn[item.index]
  qjmnFun(path)
}
</script>

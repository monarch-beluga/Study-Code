...
function getYjkjData(type){
  ...
  let url = staticData.api + param.jsonPath + `?type=${type}`
...
function getYjkjDataByName(name){
  ...
  let url = staticData.api + param.jsonPath + `?type=${curr.value}&name=${name}`
  axios.get(url).then((response) => {
    tableData = []
    let data = response.data
    let id = 0;
    data.forEach(record => {
      if (record.type === curr.value && record.name.search(name) !== -1) {
        id += 1
        let position = [record.lng, record.lat, 1000, 360, -90, 0]
        tableData.push({
          "id": id,

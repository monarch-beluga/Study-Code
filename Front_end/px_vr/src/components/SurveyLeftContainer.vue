...
async function getChart2() {
  const res = await axios.get("./api/jsonData/yjkj_StatisticData.json")
  const data = res.data
...
async function getChart3() {
  const res = await axios.get("./api/jsonData/rs_StatisticData.json")
  const data = res.data
  let colors = ["#ffff00", '#ecece9', "#ffa500"]
  let chartData = []
  let count = 0
  data.forEach((item, index) => {
    count += item.value
